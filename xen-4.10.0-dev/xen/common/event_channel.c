...
{
    struct evtchn *lchn=NULL, *rchn=NULL;
    struct domain *rd=NULL;
    int rport, ret = 0;
    
    /*
    * If the port corresponds to the shadow driver irqs,
    * induce the specific processing.
    */
    
    if(lport == VIRQ_DRV_NAPI_LAUNCHED || lport == VIRQ_DRV_NAPI_FINISHED){
        /*
        * If VIRQ_DRV_NAPI_LAUNCHED
        * Set is_running to 1 and launch timer to check completion.
        */
        printk("[SD_NIC] SD_VIRQ_INTERRUPT received %d from domain %d\n",lport,ld->domain_id);

        if(ld->domain_id != 0){
           
            if(lport == VIRQ_DRV_NAPI_LAUNCHED ){
                ld->nic_sd_handler->is_running = 1;
                printk("[SD_NIC] Launching periodic SD checker for domain %d\n",ld->domain_id);
                set_timer(ld->nic_sd_handler->timer,NOW()+SECONDS(SD_NIC_TIMEOUT));
                goto out;
            }

            /*
            * If VIRQ_DRV_NAPI_FINISHED
            * Stop timer and set is_running to 0 
            */
        
            printk("[SD_NIC] Stopping SD periodic checker for domain %d\n",ld->domain_id);
            stop_timer(ld->nic_sd_handler->timer);
            ld->nic_sd_handler->is_running = 0;
            goto out;
        }
