...
/*
* Handler Function for SD NIC monitoring
* For each epoch, we verify if the 
*   is_running , has_returned , verify
*/
void nic_sd_handler_func(void *input){

    struct domain *d = input; 
    struct vcpu *v;
    /*Does the NIC Driver poll function running ? */
    if(d->domain_id != 0 && d->nic_sd_handler->is_running){
        /* Send guest domain an VIRQ to trigger the reset of the driver */
        printk("\n[SD_MON] Timeout reached, sending VIRQ_SD_DRV_REST to domain %d",d->domain_id);
        for_each_vcpu(d,v){
            //Only send to vcpu #0
            if(!(v->vcpu_id)){
                send_guest_vcpu_virq(v,VIRQ_SD_NET_RESET);
                break;
            }
        }
                             ...
                             struct xen_arch_domainconfig *config)
{
    ...
    /*
    * Initialisation of SD structures
    * No need to initialise this for the dom0
    */

    printk("\n [SD_MON] Initialising shadow driver domid=%d \n",domid);
    //printk("\n [SD_MON] No action done for the dom0 \n");

    if(d->domain_id != 0){
        printk("\n [SD_MON] Initialising Shadow Driver Monitoring Mechanism for %d in Xen\n",d->domain_id);
        d->nic_sd_handler = xzalloc(struct sd_vm_dd_ft);
        d->nic_sd_handler->timer = xzalloc(struct timer);
        init_timer(d->nic_sd_handler->timer,nic_sd_handler_func,d,0);
        
        /*
        * For Blk devices, [coming soon]
        d->blk_sd_handler = xzalloc(struct sd_vm_dd_ft);
        d->blk_sd_handler->timer = xzalloc(struct timer);
        init_timer(d->blk_sd_handler->timer,blk_sd_handler_func,d,0);
        */
    }
