                       ...
                       uint32_t *domid, xc_domain_configuration_t *xc_config)
{
    ...
    printf("\n[xs_get_dom_path %ld] Request time and %ld microseconds",
...
retry_transaction:
    ...
    //Add timers to retrieve 
    //xenstore response time 
    xs_rm(ctx->xsh, t, dom_path);
    gettimeofday(&end,NULL);
    printf("\n[xs_rm %ld secs] Request time  and %ld microseconds",
            (end.tv_sec-start.tv_sec),
            ((((end.tv_sec-start.tv_sec)*1000000)+end.tv_usec)-(start.tv_usec)));
    
    libxl__xs_mknod(gc, t, dom_path, roperm, ARRAY_SIZE(roperm));
    gettimeofday(&end,NULL);
    printf("\n[xs_mknod %ld secs] Request time and %ld microseconds",
            (end.tv_sec-start.tv_sec),
            ((((end.tv_sec-start.tv_sec)*1000000)+end.tv_usec)-(start.tv_usec)));
    ...
    gettimeofday(&end,NULL);
    printf("\n[xs_write %ld secs] Request time and %ld microseconds",
            (end.tv_sec-start.tv_sec),
            ((((end.tv_sec-start.tv_sec)*1000000)+end.tv_usec)-(start.tv_usec)));

    rc = libxl__domain_rename(gc, *domid, 0, info->name, t);
    gettimeofday(&end,NULL);
    printf("\n[dom_rename %ld secs] Request time and %ld microseconds",
            (end.tv_sec-start.tv_sec),
            ((((end.tv_sec-start.tv_sec)*1000000)+end.tv_usec)-(start.tv_usec)));
