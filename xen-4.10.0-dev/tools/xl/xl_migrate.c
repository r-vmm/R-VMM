                           ...
                           const char *override_config_file)
{
 ...
 " responsibility to avoid that.  Sorry.\n");*/
    ...
    fprintf(stderr, "Migration failed badly, resuming at sender.\n");
    close(send_fd);

    //Here we decide to resume the instance running on the sender
    //and discard the one on the receiver side to reduce the downtime
    //due to the crash

    migration_child_report(recv_fd); //This triggers the discard of the corrupted VM on the receiver's end. 
    libxl_domain_resume(ctx, domid, 1, 0);
