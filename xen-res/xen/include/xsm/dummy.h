    ...
    xsm_default_t action, struct domain *src, struct domain *target)
{
    ...
    case XSM_PRIV:
        ...
        return -EPERM;
    case XSM_TS_PRIV:
        if (src->is_toolstack) //If it is a toolstack domain, must change domain creation for a toolstack domain 
            return 0;
        return -EPERM;
    default:
        LINKER_BUG_ON(1);
...
static XSM_INLINE int xsm_domctl(XSM_DEFAULT_ARG struct domain *d, int cmd)
{
    ...
    default:
        return xsm_default_action(XSM_PRIV, current->domain, d); //xsm_default_action(XSM_PRIV,current->domain,d) || xsm_default_action(XSM_TS_PRIV,current->domain,d)
    }
}
