...
static const char SD_NET_IRQ_NAME[] = "SD_NET_IRQ_R";
 
/*
* Variables for measuring starting time and ending time of napi mechanism
* i.e the moment when we launch the BH (softirq) and the moment when we 
* reenable interrupts for the given adapter.
*/

struct timespec64 begin_napi, end_napi;
...
/**
 * e1000_sd_reset - Called by the shadow driver to reboot the e1000 driver
 * Once reboot is okay, retrieve pending packets from netdev_sd_queue
 * and reenable interrupts
*/
irqreturn_t e1000_sd_reset(int irq, void *devid)
{
	pr_info("\n [SD_NIC] Hypervisor trigger NIC Driver reset\n");
	struct net_device *netdev = devid;
	//retrieve ethtool_ops to launch reset task

	const struct ethtool_ops *ops = netdev->ethtool_ops;

	if(ops->nway_reset(netdev)) ;

	//Submit every skb queued during recovery
	pr_info("\n [SD_NIC] Sending packets queued during reset\n");
	sd_tx_pending_skbs(netdev);
...
int e1000_open(struct net_device *netdev)
{
	...
	/* Set handler for hypervisor triggered reboot */
	pr_info("\n [SD_NIC] Setting Hypervisor triggered reboot handler\n");
	/* irq = bind_virq_to_irqhandler(
			VIRQ_SD_NET_RESET, 0 , e1000_sd_reset, 
			IRQF_PERCPU|IRQF_FORCE_RESUME|IRQF_EARLY_RESUME,
			SD_NET_IRQ_NAME, netdev);

	(void)xen_set_irq_priority(irq, XEN_IRQ_PRIORITY_MAX);*/

	if(request_irq(VIRQ_SD_NET_RESET,e1000_sd_reset,IRQF_SHARED,SD_NET_IRQ_NAME,netdev))
		return -EAGAIN; 
...
static irqreturn_t e1000_intr(int irq, void *data)
{
	...
	if (likely(napi_schedule_prep(&adapter->napi))) {
		...
		/*
		* Get the moment when the napi is being registered to be scheduled
		* & notify the hypervisor for him to keep track of the progress
		*/
		
		ktime_get_real_ts64(&begin_napi);
		notify_remote_via_irq(VIRQ_DRV_NAPI_LAUNCHED);
...
static int e1000_clean(struct napi_struct *napi, int budget)
{
	...
	if (likely(napi_complete_done(napi, work_done))) {
		...
		if (!test_bit(__E1000_DOWN, &adapter->flags)){
			...
			/*
			* Get actual time and notify Xen that the 
			* current napi call has been ended. 
			*/

			notify_remote_via_irq(VIRQ_DRV_NAPI_FINISHED);
			ktime_get_real_ts64(&end_napi);

			pr_info("\n [SD_NIC] Poll ended after : %lu ms \n",
			((end_napi.tv_sec-begin_napi.tv_sec)*1000) +
			(((end_napi.tv_nsec-begin_napi.tv_nsec)/1000)/1000)
			);
