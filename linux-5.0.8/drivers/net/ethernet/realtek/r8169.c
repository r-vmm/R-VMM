...
static const char SD_NET_IRQ_NAME[] = "SD_NET_IRQ_R_8169";
...
/*
 * r8169_sd_reset : Called by the shadow driver to reboot the r8152 driver 
 * Once reboot is okay, retrieve pending packets from netdev_sd_queue
 * and reenable interrupts 
 */
static irqreturn_t r8169_sd_reset(int irq, void *devid)
{
	pr_info("\n [SD_NIC] Hypervisor triggered NIC Driver reset %s",SD_NET_IRQ_NAME);
	...
	pr_info("\n [SD_NIC_r8169] Sending packets queued during reset \n");
...
static void rtl_reset_work(struct rtl8169_private *tp)
{
	...
	struct timespec64 begin_reset, end_reset; 
	ktime_get_real_ts64(&begin_reset);
	...
	ktime_get_real_ts64(&end_reset);

	pr_info("\n [SD_NIC_r8169] Reset ended after : %lu ms",
				((end_reset.tv_sec - begin_reset.tv_sec)*1000)+
				((end_reset.tv_nsec-begin_reset.tv_nsec)/1000000)
			);
}
...
static int rtl8169_poll(struct napi_struct *napi, int budget)
{
	...
	if (work_done < budget) {
		...
		pr_info("\n[SD_NIC_r8169] Poll ended after : %lu ms \n",
		((end_napi.tv_sec-begin_napi.tv_sec)*1000) +
		((end_napi.tv_nsec-begin_napi.tv_nsec)/1000000)
		);
...
static int rtl_open(struct net_device *dev)
{
	...
	/*Set the handler for hypervisor triggered reboot */

	pr_info("\n [SD_NIC_r8169] Setting hypervisor triggered reboot handler\n");
	/* irq = bind_virq_to_irqhandler(
		VIRQ_SD_NET_RESET, 0, r8169_sd_reset, 
		IRQF_PERCPU|IRQF_FORCE_RESUME|IRQF_EARLY_RESUME,
		SD_NET_IRQ_NAME, dev 
	);*/

	ret = request_irq(10,r8169_sd_reset,IRQF_SHARED,SD_NET_IRQ_NAME,dev);

	if(ret){
		pr_info("\n [SD_NIC_ERROR] cannot request IRQ %d",VIRQ_SD_NET_RESET);
		pr_info("\n - Code %d, EIO %d, EINVAL %d\n",ret,EIO,EINVAL);
	}
