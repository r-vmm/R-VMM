...
static const char SD_NET_IRQ_NAME[] = "SD_NET_IRQ_R_r8152";
...
/*
* r8152_sd_reset - Called by the shadow driver to reboot the r8152 driver
* Once reboot is okay, retrieve pending packets from netdev_sd_queue
* and reenable interrupts 
*/

irqreturn_t r8152_sd_reset(int irq, void *devid)
{
	pr_info("\n [SD_NIC] Hypervisor trigger NIC Driver reset %s",SD_NET_IRQ_NAME);
	...
	pr_info("\n [SD_NIC_r8152] Sending packets queued during reset\n");
...
static int r8152_poll(struct napi_struct *napi, int budget)
{
	...
	if (work_done < budget) {
		if (!napi_complete_done(napi, work_done)) goto out;

			notify_remote_via_irq(VIRQ_DRV_NAPI_FINISHED);
			ktime_get_real_ts64(&end_napi);
			pr_info("\n[SD_NIC_r8152] Poll ended after : %lu ms \n",
...
static int rtl8152_open(struct net_device *netdev)
{
	...
	pr_info("\n [SD_NIC_r8152] Setting hypervisor triggered reboot handler\n");
	/*irq = bind_virq_to_irqhandler(
		VIRQ_SD_NET_RESET, 0, r8152_sd_reset,
		IRQF_PERCPU|IRQF_FORCE_RESUME|IRQF_EARLY_RESUME,
		SD_NET_IRQ_NAME, netdev
	);
	(void)xen_set_irq_priority(irq,XEN_IRQ_PRIORITY_MAX);*/
	ret = request_irq(VIRQ_SD_NET_RESET,r8152_sd_reset,IRQF_SHARED,SD_NET_IRQ_NAME,netdev);
	//ret = 0;
	if(ret){
		pr_info("\n [SD_NIC_ERROR] cannot request IRQ 10");
...
static int rtl8152_nway_reset(struct net_device *dev)
{
	...
	pr_info("\n [SD_NIC_r8152] Reset ended after : %lu ms \n",
		((end_napi.tv_sec-begin_napi.tv_sec)*1000) +
		((end_napi.tv_nsec-begin_napi.tv_nsec)/1000000)
	);
