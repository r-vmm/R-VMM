...
static void connect(struct backend_info *be)
{
	...
	struct xenvif_queue *queue;
	struct timespec64 begin_connect, end_connect;

	/*
	* Gather information on how much time
	* it takes to reconstructs the ring buffers 
	* (how much time does it takes to reconstruct the ring buffers)
	*/

	ktime_get_real_ts64(&begin_connect);
	...
	ktime_get_real_ts64(&end_connect);

	pr_info("\n[SD_METRICS_C] : Ended after %llu ms\n",
		((end_connect.tv_sec-begin_connect.tv_sec)*1000) + 
		((end_connect.tv_nsec-begin_connect.tv_nsec)/1000000)
		);
